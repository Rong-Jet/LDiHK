...
AFFINITY_MATRIX = {
    'Adolescent_Female': {'TikTok': 0.95, 'Instagram': 0.90, 'Spotify': 0.80, 'YouTube_Shorts': 0.60, 'YouTube_Long': 0.30, 'LinkedIn': 0.00},
    'Adolescent_Male':   {'TikTok': 0.75, 'Instagram': 0.50, 'Spotify': 0.70, 'YouTube_Shorts': 0.85, 'YouTube_Long': 0.90, 'LinkedIn': 0.00},
    'Adult_Female':      {'TikTok': 0.60, 'Instagram': 0.85, 'Spotify': 0.75, 'YouTube_Shorts': 0.40, 'YouTube_Long': 0.50, 'LinkedIn': 0.60},
    'Adult_Male':        {'TikTok': 0.40, 'Instagram': 0.60, 'Spotify': 0.70, 'YouTube_Shorts': 0.50, 'YouTube_Long': 0.85, 'LinkedIn': 0.70},
    'Older_Female':      {'TikTok': 0.10, 'Instagram': 0.50, 'Spotify': 0.30, 'YouTube_Shorts': 0.10, 'YouTube_Long': 0.70, 'LinkedIn': 0.30},
    'Older_Male':        {'TikTok': 0.05, 'Instagram': 0.20, 'Spotify': 0.30, 'YouTube_Shorts': 0.10, 'YouTube_Long': 0.85, 'LinkedIn': 0.50},
}
...
def generate_user_persona():
    ...
    if 'Adolescent' in cohort: vol_mod = 1.6
    elif 'Older' in cohort: vol_mod = 0.5
    else: vol_mod = 1.0
    
    for p in active_platforms:
        if p == 'Instagram': base = 12 if sex == 'Female' else 6
        elif p == 'TikTok': base = 8 if sex == 'Female' else 5
        elif p == 'YouTube_Shorts': base = 4 if sex == 'Male' else 3
        elif p == 'YouTube_Long': base = 4 if sex == 'Male' else 2
        elif p == 'Spotify': base = 3
        elif p == 'LinkedIn': base = 2 if 'Adult' in cohort else 1
            
        persona['baselines'][p] = max(1, int(random.gauss(base, 2) * vol_mod))
        
        num_anchors = random.choice([1, 2, 3])
        if 'Adolescent' in cohort: valid_hours = [15, 16, 17, 20, 21, 22, 23] 
        elif 'Older' in cohort: valid_hours = [6, 7, 8, 12, 17, 18, 19] 
        else: valid_hours = [7, 8, 12, 17, 18, 20, 21] 
            
        persona['anchors'][p] = np.random.choice(valid_hours, size=num_anchors).tolist()
...
def generate_user_telemetry(user_id, import_id, total_days, base_date, persona):
    ...
    for day_offset in range(total_days):
        ...
        if is_weekend:
            if 'Adolescent' in persona['cohort']: weekend_mod = 1.8
            elif 'Older' in persona['cohort']: weekend_mod = 1.05
        ...
        for platform in persona['platforms']:
            ...
            daily_variance = random.gauss(0, 1.5)
            sessions = max(0, int((persona['baselines'][platform] + daily_variance) * weekend_mod))
            
            for _ in range(sessions):
                if random.random() < 0.80:
                    chosen_anchor = random.choice(persona['anchors'][platform])
                    base_ts = current_day.replace(hour=chosen_anchor, minute=0, second=0, microsecond=0)
...
def run_large_scale_seeder(total_profiles, total_days=1825):
    ...
    print(f"Starting Pipeline: {total_profiles} Clankers over {total_days} days.")
    ...
    for i in range(total_profiles):
        ...
        # NOTE: external_id matches the requested format, age/sex omitted from raw columns
...
if __name__ == "__main__":
    # Simulating 3 samples over the course of 2 days
    run_large_scale_seeder(total_profiles=3, total_days=2)